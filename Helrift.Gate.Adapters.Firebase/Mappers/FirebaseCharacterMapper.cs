...
namespace Helrift.Gate.Adapters.Firebase.Mappers;
...
internal static class FirebaseCharacterMapper
{
    ...
    public static CharacterData FromFirebase(string accountId, string charId, JsonElement el)
    {
        var c = new CharacterData
        {
            ...
            Progression = ReadProgression(el, "progression"),
    ...
    // Reads the stats flat object: { "MonstersKilled": 42, ... }
    // Returns an empty dictionary when the field is absent or malformed (backward-compatible).
    private static Dictionary<string, long> ReadCharacterStats(JsonElement root, string key)
    {
        ...
        foreach (var prop in el.EnumerateObject())
        {
            if (prop.Value.ValueKind == JsonValueKind.Number && prop.Value.TryGetInt64(out var v))
                d[prop.Name] = v;
            else if (prop.Value.ValueKind == JsonValueKind.String && long.TryParse(prop.Value.GetString(), out var sv))
                d[prop.Name] = sv;
            // unrecognised value kinds are silently skipped
        }
        return d;
    }
    ...
    {
        if (!root.TryGetProperty(key, out var el) || el.ValueKind != JsonValueKind.Object) return null;
        return new CharacterProgressionData
        {
            level = J.Int(el, "level"),
            xp = J.Long(el, "xp"),
            lifetimeXp = J.Long(el, "lifetime_xp", "lifetimeXp"),
            masteryPoints = J.Int(el, "mastery_points", "masteryPoints"),
            majesticPoints = J.Int(el, "majestic_points", "majesticPoints"),
            rebirths = J.Int(el, "rebirths"),
            divineFavour = J.Int(el, "divine_favour", "divineFavour"),
            masteryUnlocks = J.Str(el, "mastery_unlocks", "masteryUnlocks"),
            majesticUnlocks = J.Str(el, "majestic_unlocks", "majesticUnlocks"),
            weeklyDivineFavourWeekId = J.Int(el, "weekly_divine_favour_week_id", "weeklyDivineFavourWeekId"),
            weeklyCrusadeFavourEarned = J.Int(el, "weekly_crusade_favour_earned", "weeklyCrusadeFavourEarned"),
            weeklyEkFavourEarned = J.Int(el, "weekly_ek_favour_earned", "weeklyEkFavourEarned")
        };
    }
    ...
    private static object Obj(CharacterProgressionData c)
        => c == null ? null : new Dictionary<string, object>
        {
            ["level"] = c.level,
            ["xp"] = c.xp,
            ["lifetime_xp"] = c.lifetimeXp,
            ["mastery_points"] = c.masteryPoints,
            ["majestic_points"] = c.majesticPoints,
            ["rebirths"] = c.rebirths,
            ["divine_favour"] = c.divineFavour,
            ["mastery_unlocks"] = c.masteryUnlocks,
            ["majestic_unlocks"] = c.majesticUnlocks,
            ["weekly_divine_favour_week_id"] = c.weeklyDivineFavourWeekId,
            ["weekly_crusade_favour_earned"] = c.weeklyCrusadeFavourEarned,
            ["weekly_ek_favour_earned"] = c.weeklyEkFavourEarned
        };
